...
/* boston demo movement version for 8 may 25
          demo movement controls :
> default stick controls
	left stick = 8 direction movement
	right stick = rotation left/right

> horizontal slides
	left bumper = extend slides
	right bumper = retract slides
	B = claw open/close
	Y = horizontal arm in/out
	right d-pad = arm midpoint

> vertical slides
	left trigger = slide down
	right trigger = slide up
	X = bucket in/out

> other
	A = auto transfer
	left d-pad = sweeper in/out
*/

@TeleOp(name="DemoMovement", group="Linear OpMode")
public class DemoMovement extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        telemetry.addData("Status", "Initialized ^w^");
        telemetry.update();
    ...
    private void HorizontalSlideMovement() {
        ...
        boolean hsExtendBtn = gamepad1.left_bumper, hsRetractBtn = gamepad1.right_bumper;
    ...
    private void HorizontalClawAndArm() {
        ...
        boolean hClawToggleBtn = gamepad1.b; // open/close claw
        boolean hArmToggleBtn = gamepad1.y; // swing horizontal arm out/in
    ...
    private void VerticalSlideMovement() {
        ...
        double vsTriggerLeft = gamepad1.left_trigger;
        double vsTriggerRight = gamepad1.right_trigger;
        //double vsStickY = gamepad2.left_stick_y;

        if (Math.abs(vsTriggerRight - vsTriggerLeft) > joystickDeadzone) { // controls the vertical slide
            SetVSlideSpeed(linearSlideSpeed * (Math.abs(vsTriggerRight - vsTriggerLeft)) / -1);
            telemetry.addData("linear slide speed:", linearSlideSpeed * -(Math.abs(vsTriggerRight - vsTriggerLeft)) / 1);
        } else {
    ...
    private void VerticalArmAndOuttake() {
        ...
        boolean vArmToggleBtn = gamepad1.x;
   ...
   /* private void LevelTwoHang() {
    ...
    private void TransferFunction() {
        boolean transferBtn = gamepad1.a;
